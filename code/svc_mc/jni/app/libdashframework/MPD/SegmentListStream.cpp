...
{
    // This is common AVC MPD structure, each representation has its own init segment URL
    // ----------------------------------------------------------------------------------
    // if (this->segmentList->GetInitialization())
    //     return this->segmentList->GetInitialization()->ToSegment(this->baseUrls);
    // ----------------------------------------------------------------------------------

    // for SVC MPD, only one init segment which is stored at adaptation set
    // --------------------------------------------------------------------
    if (this->adaptationSet->GetSegmentList()) {
        ISegmentList* ASsegmentList = this->adaptationSet->GetSegmentList();
        return ASsegmentList->GetInitialization()->ToSegment(this->baseUrls);
    }
    // --------------------------------------------------------------------
