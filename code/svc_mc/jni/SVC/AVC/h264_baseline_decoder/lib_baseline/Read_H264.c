...
extern pthread_mutex_t StreamBufferMutex;
...
void readh264 ( const int nb_octets_to_read_temp, int NalRbspBytes, int *nb_octets_already_read, unsigned char *buffer)
{
	...
	}
	{
		...
		if ( *nb_octets_already_read + NalRbspBytes <= file_size ) {
			...
			// {
				// LOGE("[readh264] - memcpy1(buffer[%d], StreamBuffer[%d], %d)", nb_octets_to_read_temp - NalRbspBytes, *nb_octets_already_read, NalRbspBytes);
			...
			// pthread_mutex_lock(&BufferReallocMutex);
			// {
				// LOGE("[readh264] - memcpy2(buffer[%d], StreamBuffer[%d], %d)", nb_octets_to_read_temp - NalRbspBytes, *nb_octets_already_read, nb_octects);
	...
	*nb_octets_already_read += NalRbspBytes;
	...
	// LOGD("[readh264] - StreamBufferPos=%d, currentBufferPos=%d", StreamBufferPos, currentBufferPos);
}
 ...
 *
 * @param NbBytesToRead:
 *		copy the amount of bytes from StreamBuffer to buffer in each run.
 * @param *buffer:
 *		the buffer store the data will be decoded.
 * @param ReadNewBytes:
 *		get the amount of bytes in this run.
 * @param *NbBytesTreated:
 *		how many bytes already be got by decoder.
 */
void ReadAuH264 (int NbBytesToRead, unsigned char *buffer, int ReadNewBytes, int *NbBytesTreated)
{
	extern int availableDataSize;
	// LOGE("READAu264 : BEFORE NbBytesToRead = %d, ReadNewBytes = %d, *NbBytesTreated = %d", NbBytesToRead, ReadNewBytes, *NbBytesTreated);

	if (NbBytesToRead < ReadNewBytes) {
		LOGE("[ReadAuH264] : NbBytesToRead = %d < ReadNewBytes = %d", NbBytesToRead, ReadNewBytes);
	}
	...
	if(ReadNewBytes)
	{
		...
		// LOGE("[ReadAuH264] - memcpy(buffer[0], buffer[%d], %d)", ReadNewBytes, NbBytesToRead - ReadNewBytes);
		...
		if(BytesToRead) {
			...
			if(*NbBytesTreated + ReadNewBytes >= file_size) {
				int first_part = file_size - *NbBytesTreated -1;
				...
				// LOGE("[ReadAuH264] - memcpy1(buffer[%d], StreamBuffer[%d], %d)", NbBytesToRead - ReadNewBytes, *NbBytesTreated, first_part);
				memcpy(&buffer[NbBytesToRead - ReadNewBytes], &StreamBuffer[*NbBytesTreated], first_part * sizeof(char));

				// LOGE("[ReadAuH264] - memcpy2(buffer[%d], StreamBuffer[0], %d)", NbBytesToRead - ReadNewBytes + first_part, ReadNewBytes - first_part);
				memcpy(&buffer[NbBytesToRead - ReadNewBytes + first_part], StreamBuffer, (ReadNewBytes - first_part) * sizeof(char));

				availableDataSize -= ReadNewBytes;
				// LOGE("[ReadAuH264] - availableDataSize => from %d to %d (-%d)", availableDataSize+ReadNewBytes, availableDataSize, ReadNewBytes);
				// pthread_mutex_unlock(&StreamBufferMutex);
				// }
				// pthread_mutex_unlock(&BufferReallocMutex);
				// LOGE("SYNC : EXIT BRM in ReadAuH264() with datasize = %d", availableDataSize);
			}
			else {
				...
				// pthread_mutex_lock(&StreamBufferMutex);
				// LOGE("[ReadAuH264] - memcpy(buffer[%d], StreamBuffer[%d], %d)", NbBytesToRead - ReadNewBytes, *NbBytesTreated, BytesToRead);
			...
			// 	availableDataSize = StreamBufferPos - currentBufferPos + 1;
			// }
			// else {
			// 	availableDataSize = (file_size + 1) - (currentBufferPos - StreamBufferPos);
			// }

			// availableDataSize = (StreamBufferPos - currentBufferPos + 1) % file_size;
			...
			// 	// LOGE("test_buffer : data size = %d with SBP >= CBP", availableDataSize);
			// }
			// else {
			// 	pthread_mutex_lock(&DataSizeMutex);
			// 	{
			...
			// LOGE("[READAu264] - Current available data size is %d", availableDataSize);
...
#else
			currentBufferPos = *NbBytesTreated = 0;
#endif
		}
	}
	// LOGD("[READAu264] - StreamBufferPos=%d, currentBufferPos=%d", StreamBufferPos, currentBufferPos);
}
#endif
