...
#define  LOG_TAG0n    __FILE__
#define  LOGV0n(...)  __android_log_print(ANDROID_LOG_VERBOSE, LOG_TAG0n, __VA_ARGS__)
#define  LOGD0n(...)  __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG0n, __VA_ARGS__)
#define  LOGI0n(...)  __android_log_print(ANDROID_LOG_INFO, LOG_TAG0n, __VA_ARGS__)
#define  LOGW0n(...)  __android_log_print(ANDROID_LOG_WARN, LOG_TAG0n, __VA_ARGS__)
#define  LOGE0n(...)  __android_log_print(ANDROID_LOG_ERROR, LOG_TAG0n, __VA_ARGS__)
...
static __inline void luma_sample_interp_1_n(unsigned char image [RESTRICT],	unsigned char refPicLXl[RESTRICT], const short PicWidthSamples, const short stride)
{
	...
	for (j = 0; j < 4; j++, image += stride, refPicLXl += PicWidthSamples){		
		for (i = 0; i < 4; i++)	{
			// if (!refPicLXl) LOGE0n("[%s]:%d - refPicLXl is NULL", __func__, __LINE__);
