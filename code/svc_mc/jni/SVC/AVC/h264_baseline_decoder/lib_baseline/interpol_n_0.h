...
#define  LOG_TAGn0    __FILE__
#define  LOGVn0(...)  __android_log_print(ANDROID_LOG_VERBOSE, LOG_TAGn0, __VA_ARGS__)
#define  LOGDno(...)  __android_log_print(ANDROID_LOG_DEBUG,   LOG_TAGn0, __VA_ARGS__)
#define  LOGIno(...)  __android_log_print(ANDROID_LOG_INFO,    LOG_TAGn0, __VA_ARGS__)
#define  LOGWno(...)  __android_log_print(ANDROID_LOG_WARN,    LOG_TAGn0, __VA_ARGS__)
#define  LOGEno(...)  __android_log_print(ANDROID_LOG_ERROR,   LOG_TAGn0, __VA_ARGS__)
...
static __inline void luma_sample_interp_n_0(unsigned char ptr_ref [RESTRICT], unsigned char ptr_imge [RESTRICT], const short PicWidthSamples, const short stride){
	...
	for (k = 0; k < 4; ptr_ref += PicWidthSamples, ptr_imge += stride, k++) {
      ...
      // if (!ptr_ref) LOGEno("[%s]:%d - ptr_ref is NULL", __func__, __LINE__);
		unsigned char A = ptr_ref[0]; // crash
