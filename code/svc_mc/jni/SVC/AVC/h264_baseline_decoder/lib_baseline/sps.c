...
void seq_parameter_set_data ( unsigned char *data, int *position,  SPS *pt_sps)
{
  ...
  // LOGD("[seq_parameter_set_data:show] - Width start at %d = %d", (*position)-1, pt_sps->pic_width_in_mbs<<4);
  pt_sps -> pic_height_in_map_units = (short) read_ue(data, position) + 1;
  // LOGD("[seq_parameter_set_data:show] - Height start at %d = %d", (*position)-1, pt_sps->pic_height_in_map_units<<4);
...
void seq_parameter_set( unsigned char *data, SPS *sps){
  ...
  unsigned char constraint = (unsigned char) getNbits(data, &position, 8); //constraint_set0_flag ~ constraint_set4_flag + reserved_zero_4bits
  ...
  if (profile_idc && !(constraint & 0xf)){
		...
		char SeqParamSetId = CLIP3(0, SPS_BUF_SIZE, Sps_id); //CLIP3(X,Y,Z) = MIN(Y,MAX(X,Z))
		SPS *pt_sps = &sps [SeqParamSetId];
    // LOGI("[seq_parameter_set:show] - sps[%d]", SeqParamSetId);
