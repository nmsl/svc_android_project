...
*/
void dec_ref_pic_marking ( unsigned char *data, int *Position, int nal_unit_type, SPS *sps, LAYER_MMO *LayerMmo)
{
	// LOGD("[dec_ref_pic_marking] - *data=%p, Position=%p, nal_unit_type=%d, sps=%p, LayerMmo=%p", data, Position, nal_unit_type, sps, LayerMmo);

	if ( nal_unit_type == 5 ) {
		// LOGD("[dec_ref_pic_marking] - nal_unit_type == 5");
		...
		// LOGD("[dec_ref_pic_marking] - long_term_reference_flag = %d", long_term_reference_flag);
