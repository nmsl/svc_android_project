							   ...
							   const int ai_iN,	const VLC_TABLES *vlc, unsigned char NonZeroCountCache [ RESTRICT ])
{
	...
	// if (!NonZeroCountCache) LOGE("[%s]:%d - NonZeroCountCache is NULL", __func__, __LINE__);
	...
	// For debugging
	// if (tmp < 0 || tmp >= 17) LOGE("[%s]:%d - tmp=%d [0-16]", __func__, __LINE__, tmp);
	int ddindex = getCoeffTokenIndex(tmp);
	// if (ddindex < 0 || ddindex >= 4) LOGE("[%s]:%d - getCoeffTokenIndex(tmp)=%d [0-3]", __func__, __LINE__, ddindex);
	ddindex = vlc->Coeff_token_vlc_adress[getCoeffTokenIndex(tmp)];
	if (ddindex < 0 || ddindex >= 1388) LOGE("[%s]:%d - vlc->Coeff_token_vlc_adress[getCoeffTokenIndex(tmp)]=%d [0-1387]", __func__, __LINE__, ddindex);
	int coeff_token = pstTable [index] . code; // crash
