...
#define  LOG_TAG    "SliceHeaderExtension"
									...
									LIST_MMO *RefPicListL0, LIST_MMO *RefPicListL1)
{
	...
	if (Nal -> QualityId == 0){
		...
		if( Nal -> NalRefIdc != 0 ) {
			// LOGD("[SvcReadReferenceListProcessing] - Before dec_ref_pic_marking()");
			dec_ref_pic_marking(data, position, Nal -> IdrFlag * 5, pt_sps_id, LayerMmo);
			// LOGD("[SvcReadReferenceListProcessing] - After  dec_ref_pic_marking()");
			if (!pt_sps_id -> slice_header_restriction_flag){
				// LOGD("[SvcReadReferenceListProcessing] - !pt_sps_id -> slice_header_restriction_flag");
				Nal -> StoreRefBasePicFlag = getNbits(data, position, 1); //
				if (( Nal -> UseRefBasePicFlag || Nal -> StoreRefBasePicFlag) && !Nal -> IdrFlag){
					// LOGD("[SvcReadReferenceListProcessing] - Before DecRefBasePicMarking()");
					DecRefBasePicMarking(data, position, Mmo, pt_sps_id, Nal);
					// LOGD("[SvcReadReferenceListProcessing] - After  DecRefBasePicMarking()");
				}
			}
			// LOGD("[SvcReadReferenceListProcessing] - Before SetDecRefLayer()");
			SetDecRefLayer(Nal, Mmo, LayerMmo, Nal -> DependencyId);
			// LOGD("[SvcReadReferenceListProcessing] - After  SetDecRefLayer()");
		}else {
			LayerMmo -> index = 0;
		}
	}else {
...
void NalParamInit(NAL *Nal, PPS *pps, SPS *sps, SPS *pt_sps_id)
{
	...
	// LOGD("[NalParamInit] - Nal->QualityId=%d", Nal->QualityId);
	if( Nal -> QualityId){
	...
	// Jargo
	// LOGD("[NalParamInit:show] - Up Frame Size = %dx%d", Nal->up_Width, Nal->up_Height);
	// LOGD("[NalParamInit:show] - Down Frame Size = %dx%d", Nal->down_Width, Nal->down_Height);
	// LOGD("[NalParamInit] - base_pps = pps[Nal->pic_parameter_id[Nal->BaseLayerId=%d]=%d]", Nal->BaseLayerId, Nal->pic_parameter_id[Nal->BaseLayerId]);
	// LOGD("[NalParamInit] - base_sps = sps[base_pps->seq_parameter_set_id=%d + (Nal->BaseLayerId ? 16 : 0)=%d]", base_pps->seq_parameter_set_id, (Nal->BaseLayerId ? 16 : 0));
