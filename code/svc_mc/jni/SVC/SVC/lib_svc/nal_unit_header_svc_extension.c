...
#define  LOG_TAG    "NALUnitHeaderSVCExtension"
...
void NalUnitData(unsigned char *Data, int *position, NAL *Nal, int *EndOfSlice)
{
	...
	// LOGI("[NalUnitData] - DId=%d,QId=%d,DqId=%d,TId=%d",Nal->DependencyId, Nal->QualityId, Nal->DqId, Nal->TemporalId);
