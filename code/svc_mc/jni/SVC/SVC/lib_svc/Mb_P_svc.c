			   ...
			   const STRUCT_PF *vectors, REFPART RefPartition[16])
{
	...
	for ( mbPartIdx = 0 ; mbPartIdx < 4 ; mbPartIdx++ ) {
		...
		if (mode < 0 || mode >= 4) LOGE("[%s]:%d mode=%d [0-4]", __func__, __LINE__, mode);
		decode_sub [mode](mv_cache, ref_cache, Block, mbPartIdx, mv_l0, ref_bas_l0, Nal, &RefPartition[((mbPartIdx & 2) << 2) + ((mbPartIdx & 1) << 1)]); // crash
	}
