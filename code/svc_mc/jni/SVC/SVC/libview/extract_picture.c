...
{
	// LOGI("[ExtractPicture_C] - xsize=%d, ysize=%d", xsize, ysize);

	int i,j;
	
	int Right  = Crop & 0xff; // 0
	int Left   = (Crop >> 8) & 0xff; // 0
	int Bottom = (Crop >> 16) & 0xff; // 12(320*180), 8(640x360), 0(1280x720)
	int Top    = (Crop >> 24) & 0xff; // 0
	// LOGI("[ExtractPicture_C] - Right=%d, Left=%d, Bottom=%d, Top=%d", Right, Left, Bottom, Top);
	int Xsize = xsize + 2 * edge + Right + Left; // Xsize = 320 + 2*16 + 0 = 352

	const int offset_L = (Xsize * (16 + Top) + Left + edge + address_pic); // Xsize*16 + 16 + address_pic
	const int offset_C = Xsize * (4 + Top / 4) + (Left / 2) + (edge / 2) + (address_pic >> 2); // Xsize*1 + 0 + 8 + address_pic/4

	// LOGI("[ExtractPicture_C] - Xsize=%d, offset_L=%d, offset_C=%d", Xsize, offset_L, offset_C);
	...
	// LOGI("[ExtractPicture_C] - im_in=%p, im_in_Cb=%p, im_in_Cr=%p", im_in, im_in_Cb, im_in_Cr);

	
	int y_limit = ysize; // y_limit = 180
	int x_limit = xsize >> 2; // x_limit = 320/4 = 80
	int stride = (Xsize - Right - Left); // stride = 320+32 = 352
	Xsize = Xsize >> 2; // Xsize = 88
	// LOGI("[ExtractPicture_C] - limit[x, y]=[%d, %d], stride=%d, Xsize=%d", x_limit, y_limit, stride, Xsize);


	for (i = 0; i < y_limit; i++, im_in += Xsize){
		unsigned int *im_out = (unsigned int *) &img_luma_out[i * stride];
		// LOGI("[ExtractPicture_C] - im_out[%d-%d]", i*stride, i*stride+x_limit-1);
		for (j = 0; j < x_limit; j++){
			im_out[j] = im_in[j];
		}
	}
	...
	for (i = 0; i < y_limit; i++){
		...
		// LOGI("[ExtractPicture_C] - im_out_Cb&im_out_Cr[%d-%d]", i*stride, i*stride+x_limit-1);
		for (j = 0; j < x_limit; j++){
