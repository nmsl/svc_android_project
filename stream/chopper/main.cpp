...
int extract_layers(FILE *infile) // I modified SJ Park(Korea University)'s code -by ES RYU-
{
    ...
    while ((temp = fgetc(infile)) != -1) {
        ...
        switch (status) {
            ...
            case 2:
                {
                    // NAL Header first 3 byte = "0x00 00 00"
                    if (temp == 0x00)  {
                        status++;
                    }
                    else {
            ...
            case 3:
                {
                    // NAL Header 4th byte = '0x01'
                    if (temp == 0x01)  {
                        // deal with the first NAL Unit (No need to output offset)
                        if (offset != 4)  {
            ...
            // 1st byte of NAL Header [F(1) NRI(2) Type(5)]
            case 4:
                {
                    forb = (temp & 0x80) >> 7;
                    nri = (temp & 0x60) >> 5;
                    ...
                    #ifdef DEBUG
                    printf("No=%ld,\tOffset=%ld,\tF=%d,\tNRI=%d,\tType=(%2d) %s,\n", index, offset - 5, forb, nri, type, typeText);
                    #endif
            ...
            // 2nd byte of NAL Header [R(1) I(1) PRID(6)]
            case 5:
                {
                    ...
                    #ifdef DEBUG
                    printf("\tR=%d, I=%d, PRID=%d,\n", reserved_one_bit, idr_flag, priority_id);
                    #endif
            ...
            case 6: //3rd byte of NAL Header [N(1) DID(3) QID(4)]
                {
                    ...
                    #ifdef DEBUG
                    printf("\tN=%d, DID=%d, QID=%d\n", no_inter_layer_pre_flag, dependency_id, quality_id);
                    #endif
            ...
            case 7: //4th byte of NAL Header [TID(3) U(1) D(1) O(1) RR(2)]
                {
                    ...
                    #ifdef DEBUG
                    printf("\tTID=%d, U=%d, D=%d, O=%d, RR=%d\n",
                        temporal_id, use_ref_base_pic_flag, discardable_flag, output_flag, reserved_three_2bits);
                    #endif
...
int chop_segment(FILE *fin, char* out_file_folder, char* prefix, int segment_no, int layerID) {
    ...
    for (int i = 0; i < NalUnitAnchorList.size(); ++i) {
        // TYPE 6 = SEI, TYPE 20 = CS S Ext, TYPE 5 = CS IDR //
        if (NalUnitAnchorList[i].type == 6 && NalUnitAnchorList[i + 3].type == 20) {
            if (NalUnitAnchorList[i + 2].type == 5) IDRcounter++;
            ...
            if (NalUnitAnchorList[i + 1].tid < 5) {
                ...
                if (layerID >= 1) NalUnitAnchor::write(fin, fout, NalUnitAnchorList[i + 4]);
                if (layerID >= 2) NalUnitAnchor::write(fin, fout, NalUnitAnchorList[i + 5]);
            }
            i += 5;
        }
        else if (withHeader) {
...
/*
 * argv[1] = mode (0: All segments, 1: Specific Segment, 2: parsing Mode)
