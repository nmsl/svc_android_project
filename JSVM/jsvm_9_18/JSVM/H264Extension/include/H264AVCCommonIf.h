...
public:
  ...
  MyList& operator += ( const MyList& rcMyList) { if( ! rcMyList.empty() ) { this->insert( this->end(), rcMyList.begin(), rcMyList.end());} return *this; } // Modify By Jargo
  ...
  Void pushBack( const T& rcT )         { if( sizeof(T) == sizeof(void*) ) { if( rcT != NULL ){ this->push_back( rcT);} } } // Modify By Jargo
  Void pushFront( const T& rcT )        { if( sizeof(T) == sizeof(void*) ) { if( rcT != NULL ){ this->push_front( rcT);} } } // Modify By Jargo
